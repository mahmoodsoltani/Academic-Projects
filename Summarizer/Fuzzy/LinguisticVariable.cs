...
namespace MyLib.Fuzzy
{
    ...
    public class LinguisticVariable
    {
        ...
        /// Returns an existing label from the linguistic variable.
        /// </summary>
        /// <param name="labelIndex">Number of the label to retrieve.</param>
        /// <returns>Reference to named label (<see cref="FuzzySet"/>).</returns>
        public FuzzySet GetLabel(int labelIndex)
        {
            int index = 0;
            foreach (KeyValuePair<string, FuzzySet> label in labels)
                if (index == labelIndex)
                    return label.Value;
                else index++;
            return null;
        ...
        /// <summary>
        /// return all Labels (Membership Functions)
        /// </summary>
        /// <returns></returns>
        public Dictionary<string, FuzzySet> GetLabels()
        {
            return labels;
        }
