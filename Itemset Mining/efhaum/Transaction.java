...
public class Transaction {
    ...
    public Transaction(int[] items, int[] utilities, int transactionUtility) {
    	...
    	this.transactionAUtility = transactionUtility;
    	this.offset = 0;
    ...
    public Transaction(Transaction transaction, int offsetE) {	
    	...
    	//=========AHU
    	//this.transactionAUtility = transaction.transactionAUtility - utilityE;
    	...
    	//for(int i = transaction.offset; i < offsetE; i++){
    		//this.transactionAUtility -= transaction.utilities[i];
    	//}
    	transactionAUtility=utilities[offsetE+1];
    	for(int k = offsetE+1; k < utilities.length; k++){
			if(transactionAUtility <  utilities[k]){
				transactionAUtility =  utilities[k];
			}
		}
     ...
     */
     public String toString() {
		StringBuilder buffer = new StringBuilder();
		 ...
		 buffer.append(" Remaining Utility:" +transactionAUtility);
	...
	public void removeUnpromisingItems(int[] oldNamesToNewNames) {
    	...
    	for(int j=0; j< items.length;j++) {
    		...
    		if(newName != 0) {
    			...
    			// else this utility is the max change it transactionAUtility
    			if (transactionAUtility == utilities[j])
    				sw=true;
    		}
    	}
    	...
    	if (sw && i!=0){
    		transactionAUtility=this.utilities[0];
    		for(int k = 1; k < utilities.length; k++){
    			if(transactionAUtility <  utilities[k]){
    				transactionAUtility =  utilities[k];
    			}
    		}
