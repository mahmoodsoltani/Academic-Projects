package   ca.pfv.spmf.algorithms.frequentpatterns.efimn;
 ...
 */
public class AlgoEFIMN {

	//===================== EFIMN ===========================
	private static Set<Integer> negativeItems = null;
	//====================================================
    ...
    public Itemsets runAlgorithm(int minUtil, String inputPath, String outputPath, boolean activateTransactionMerging, int maximumTransactionCount, boolean activateSubtreeUtilityPruning) throws IOException {
		...
		// read the input file
		//=====================modified for EFIMN ===========================
		DatasetN dataset = new DatasetN(inputPath, maximumTransactionCount, negativeItems);
	  ...
	  // Sort promising items according to the increasing order of TWU
       ...
       for (int j=0; j< itemsToKeep.size(); j++)
	   {
    	...
    	{
    		// Get the transaction
    		TransactionN transaction  = dataset.getTransactions().get(i);
    	...
    	if(activateTransactionMerging){
    		...
    		Collections.sort(dataset.getTransactions(), new Comparator<TransactionN>(){
				@Override
				/**
...
//    	//======
	...
	public static void insertionSort(List<Integer> items, int [] utilityBinArrayTWU){
		...
		for(int j=1; j< items.size(); j++){
			...
			//====================== EFIMN =======================
			// we update insertion sort such that all negative items succeed positive items
			int comparison=0;
			Boolean itemIIsNegative = negativeItems.contains(itemI);
			Boolean itemJIsNegative = negativeItems.contains(itemJ);
			if(!itemIIsNegative && itemJIsNegative) {
				comparison=-1;
			}else if (itemIIsNegative && !itemJIsNegative)  {
				comparison=1;
			//====================================================
			}else{
			...
			while(comparison > 0){
				...
				//====================== EFIMN =======================
				// we update insertion sort such that all negative items succeed positive items
				itemIIsNegative = negativeItems.contains(itemI);
				if(!itemIIsNegative && itemJIsNegative) {
					comparison= -1;
				}else if (itemIIsNegative && !itemJIsNegative)  {
					comparison= 1;
     ...
     */
    private void backtrackingEFIM( List<TransactionN> transactionsOfP,
    		List<Integer> itemsToKeep, List<Integer> itemsToExplore, int prefixLength, int prefixPositiveUtility) throws IOException {
		...
		for (int j = 0; j < itemsToExplore.size(); j++) {
			...
			//-------------------EFIMN----------------
			// if pu(P)<minUtil if we extend pattern P with a negative item it's utility  will be also smaller than minutil
			
			if (prefixPositiveUtility < minUtil && negativeItems.contains(newNamesToOldNames[e]))
				break;
			//-------------------------------
	        ...
	        List<TransactionN> transactionsPe = new ArrayList<TransactionN>();
	        
	        // variable to calculate the utility of P U {e}
			int utilityPe = 0;		
			
			//-------------------EFIMN----------------
			int positiveUtilityPe = 0;		
			//-------------------------------
			...
			TransactionN previousTransaction = null;
			...
			// For each transaction
	        for(TransactionN transaction : transactionsOfP) {
	            ...
	            if (positionE > -1  ) { 
					...
					if(transaction.getLastPosition() == positionE){
						...
						//-------------------EFIMN----------------
						if (!negativeItems.contains(e))
							positiveUtilityPe += transaction.utilities[positionE] + transaction.positivePrefixUtility;
						else
							positiveUtilityPe +=transaction.positivePrefixUtility;;
						//-------------------------------
		            	...
		            	if(activateTransactionMerging && MAXIMUM_SIZE_MERGING >= (transaction.items.length - positionE)){
							...
							TransactionN projectedTransaction = new TransactionN(transaction, positionE);
							utilityPe  += projectedTransaction.prefixUtility;
							
							//-------------------EFIMN----------------
							positiveUtilityPe += projectedTransaction.positivePrefixUtility;
							//-------------------------------
							...
							if(previousTransaction == null){
								...
								if(consecutiveMergeCount == 0){
									...
									previousTransaction = new TransactionN(items, utilities, previousTransaction.transactionUtility + projectedTransaction.transactionUtility);
	    	...
	    	if(activateSubtreeUtilityPruning){
	    		...
	    		backtrackingEFIM(transactionsPe, newItemsToKeep, newItemsToExplore,prefixLength+1,positiveUtilityPe);
	    	}else{
	    		// if sub-tree utility pruning is deactivated, we consider secondary items also
	    		// as primary items
	    		backtrackingEFIM(transactionsPe, newItemsToKeep, newItemsToKeep,prefixLength+1,positiveUtilityPe);
	    	}
		}
     ...
     */
    private boolean isEqualTo(TransactionN t1, TransactionN t2) {
	 ...
	 */
	public void useUtilityBinArrayToCalculateLocalUtilityFirstTime(DatasetN dataset) {
		...
		// For each transaction
		for (TransactionN transaction : dataset.getTransactions()) {
			// for each item
	 ...
	 * @param dataset the transaction database
	 */
	public void useUtilityBinArrayToCalculateSubtreeUtilityFirstTime(DatasetN dataset) {
     ...
     */
    private void useUtilityBinArraysToCalculateUpperBounds(List<TransactionN> transactionsPe, 
		...
		// for each transaction
		for (TransactionN transaction : transactionsPe) {
			...
			for (int i = transaction.getItems().length - 1; i >= transaction.offset; i--) {
				...
				if (contains) {
					...
					if (utilityBinArrayLU[item]<0 && utilityBinArrayLU[item]>0 ){
	...
	public void printStats() {
		...
		System.out.println("========== EFIMN - STATS ============");
