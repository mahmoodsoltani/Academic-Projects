...
import ca.pfv.spmf.algorithms.frequentpatterns.efimPlus.Transaction;
 ...
 */
 public class AlgoEFIMPlus {
     ...
     */
    public Itemsets runAlgorithm(int minUtil, String inputPath, String outputPath, boolean activateTransactionMerging, int maximumTransactionCount, boolean activateSubtreeUtilityPruning,int MaxItemSetLen) throws IOException {
		...
		// read the input file
		Dataset dataset = new Dataset(inputPath, maximumTransactionCount,MaxLen);
      ...
      // Arrays.fill(newNamesToOldNames,-1);
...
//    	//======
    ...
    private void backtrackingEFIM( List<Transaction> transactionsOfP,
	            ...
	            if (positionE > -1  ) { 
					...
					if(transaction.getLastPosition() == positionE){
		            	...
		            	if(activateTransactionMerging && MAXIMUM_SIZE_MERGING >= (transaction.items.length - positionE)){
							...
							Transaction projectedTransaction = new Transaction(transaction, positionE,MaxLen);
							...
							if(previousTransaction == null){
								...
								if(consecutiveMergeCount == 0){
									...
									previousTransaction = new Transaction(items, utilities, previousTransaction.transactionUtility + projectedTransaction.transactionUtility,prefixLength);
   	...
   	if (prefixLength +1<MaxLen){
	    		
		//==== Next, we will calculate the Local Utility and Sub-tree utility of
	        // all items that could be appended to PU{e} ====
	        useUtilityBinArraysToCalculateUpperBounds(transactionsPe, j, itemsToKeep,prefixLength);  
    	   ...
    	   for(int i = transaction.getItems().length-1; i >=0; i--) {
    		   ...
    		   //=================modify according to EFIM plus
    		    // we add the utility of the current item to its sub-tree utility
       		   ...
       		   for (int j=0;j<MaxLen-1&& j<len;++j)
       		      utilityBinArraySU[item]+=tempUtil[j];
			...
			for (int i = transaction.getItems().length - 1; i >= transaction.offset; i--) {
				...
				if (contains) {
					...
					utilityBinArraySU[item] += transaction.getUtilities()[i]+transaction.prefixUtility;
		      		for (int k=0;k<MaxLen-prefixLength-1&& k<len;++k)
		        		      utilityBinArraySU[item]+=tempUtil[k];
		     		   // we add the utility of the current item to its sub-tree utility
           		    len=sortedInsert(tempUtil,len,transaction.getUtilities()[i]);
           		    //============================================================
     ...
     */
    public void SaveResult(BufferedWriter writer1,int minUtil,int MaxLen)
    {
    	try {
			
			writer1.write(minUtil+","+MaxLen+","+patternCount+","+(endTimestamp - startTimestamp)+","+  MemoryLogger.getInstance().getMaxMemory());
			writer1.flush();
			writer1.newLine();
		} catch (IOException e) {
